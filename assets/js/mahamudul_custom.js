class WizardForm {
  ...
  updateUI() {
    ...
    // After UI update, re-attach project remove button listeners
  ...
  handleProjectRemoveClick(e) {
    ...
    // Get the project title displayed inside the card's h5
    const title = projectCard.querySelector("h5")?.textContent.trim();
    if (!title) return;

    // Find project id by matching title (assuming unique titles)
    // Better to store id in dataset, but since not done, fallback to matching title
      ...
      case 0:
        ...
        this.formData.personalInfo = {
          ...
          documents: Array.from(
            document.querySelectorAll("#uploadedFiles .uploaded-file")
      ...
      case 1:
        this.formData.credentials = {
          ...
          certifications: Array.from(
            document.querySelectorAll("#uploadedCertifications .uploaded-file")
          ).map((el) => el.textContent.replace("×", "").trim()),
        };
        break;
    }
  ...
  renderProjects() {
    ...
    // Attach IDs as data attribute for easier removal
